...
/**
 * `/wormhole11` — **Cosmic** behind **wormhole5** Julia stack, with **wormhole9** journey intro
 * (`journeyCameraFromStart`, `helixWallInsetMul` 1.0, micro-intro + preloader). Isolated preview route.
 */
export function Wormhole11ClientShell({ children }: { children: ReactNode }): ReactElement {
  ...
  useLayoutEffect(() => {
    ...
    tunnelStore.setState({
      ...
      depth: touchPrimary ? WORMHOLE6_MOBILE_TUNNEL_START.depth : WORMHOLE11_TUNNEL_START.depth,
      velocity: touchPrimary ? WORMHOLE6_MOBILE_TUNNEL_START.velocity : WORMHOLE11_TUNNEL_START.velocity,
      scrollInputIdle: 1,
  ...
  return (
    <div className="relative min-h-[100dvh] w-full bg-[#030208]">
      <p
        className="pointer-events-none fixed left-1/2 top-[4.25rem] z-[90] max-w-[min(90vw,28rem)] -translate-x-1/2 rounded-md border border-teal-500/40 bg-black/80 px-2.5 py-1 text-center text-[10px] font-medium uppercase tracking-[0.08em] text-teal-100/90 shadow-lg backdrop-blur-sm"
        aria-hidden
      >
        Wormhole 11 preview — cosmic + wormhole5 + wormhole9 intro cam
      </p>
      <CosmicBackdrop />
