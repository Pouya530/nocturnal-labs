 ...
 * no `helixLabFullscreen`) **plus** `journeyCameraFromStart` so `wormholeHomeIntroCam01` from the home micro-intro
 * scales journey FOV/dolly (`JuliaWormholeBackdrop`). Home tunnel chrome, preloader, and atmosphere gate unchanged
...
export function Wormhole8ClientShell({ children }: { children: ReactNode }): ReactElement {
  ...
  const wormhole8HelixBoostEnabled = useSyncExternalStore(
    ...
    () => tunnelStore.getState().wormhole8HelixBoostEnabled,
    () => true,
  );
  ...
  return (
    <div className="relative min-h-[100dvh] w-full bg-[#030208]">
      <p
        ...
        aria-hidden
      >
        Wormhole 8 preview — wormhole5 GL + journeyCameraFromStart (intro cam ramp)
      </p>
      <WormholeJuliaThreeBackdrop
        ...
        helixWallInsetMul={wormhole8HelixBoostEnabled ? 5 : 0.88}
      />
